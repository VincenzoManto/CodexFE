...
function createTableHead(name: string): HTMLTableHeaderCellElement {
  ...
  const headingThRemove = document.createElementNS(constant.nsHtml, "th");
  headingThRemove.setAttributeNS(null, "colspan", `${2}`);
  headingThRemove.setAttributeNS(null, "style", `text-align:right`);
  headingThRemove.innerHTML = `<i class="circle">P</i>`;
  headingTh.innerHTML = name;
  headingTr.appendChild(headingTh);
  headingTr.appendChild(headingThRemove);
